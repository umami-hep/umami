...
def get_validation_dict_name(WP_b, fc_value, n_jets, dir_name, plot_label=""):
    return os.path.join(dir_name, f"validation_WP{str(WP_b).replace('.','p')}_fc{str(fc_value).replace('.','p')}_{n_jets}jets_Dict.json")
...
def get_parameters_from_validation_dict_name(dict_name):
    ...
    if get_validation_dict_name(**parameters) != dict_name:
        raise Exception(f"Can't infer parameters correctly for {dict_name}. Parameters: {parameters}")
    return parameters
...
def setup_output_directory(dir_name):
    ...
    if outdir.is_dir():
        ...
        raise Exception(f"{dir_name} is the output directory name but it already exists as a file!")
    else:
        outdir.mkdir()
...
class MyCallbackUmami(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.val_data_dict:
            result_dict=evaluate_model(self.model, self.val_data_dict, self.target_beff, self.charm_fraction)
...
def get_jet_feature_indicies(exclude: list = []):
    ...
    for exclude_this in exclude:
        if exclude_this in algo_indicies:
            print(f"INFO: Excluding {exclude_this} from the jet feature inputs!")
...
def GetTestFile(file: str, var_dict: str, preprocess_config: dict, nJets: int, exclude: list):
    ...
    X, Y = GetTestSample(
        input_file=file,
        var_dict=var_dict,
        preprocess_config=preprocess_config, nJets=nJets, exclude=exclude)
...
def evaluate_model(model, data_dict, target_beff=0.77, cfrac=0.018):
    ...
    c_rej_dips, u_rej_dips = GetRejection(y_pred_dips, data_dict["Y_valid"], target_beff, cfrac)
    c_rej_umami, u_rej_umami = GetRejection(y_pred_umami, data_dict["Y_valid"], target_beff, cfrac)
    ...
    if data_dict["X_valid_add"] is not None:
        ...
        c_rej_dips_add, u_rej_dips_add = GetRejection(
            y_pred_dips_add, data_dict["Y_valid_add"],
            target_beff, cfrac
        )
        c_rej_umami_add, u_rej_umami_add = GetRejection(
            y_pred_umami_add, data_dict["Y_valid_add"],
            target_beff, cfrac
        )
    result_dict = {
        "val_loss": loss,
...
def calc_validation_metrics(train_config, preprocess_config, target_beff=0.77, cfrac=0.018, nJets=300000):
    val_data_dict = load_validation_data(train_config, preprocess_config, nJets)
    training_output = [
        os.path.join(train_config.model_name, f)
        for f in os.listdir(train_config.model_name)
        if "model" in f
    ]
    with open(get_validation_dict_name(**train_config.Eval_parameters_validation,
                dir_name=train_config.model_name), "r") as training_out_json:
    ...
    for n, model_file in enumerate(training_output):
        ...
        result_dict = {}
        epoch= int(model_file[model_file.find("epoch") + 5 : model_file.find(".h5")])
        ...
        val_result_dict = evaluate_model(umami, val_data_dict, target_beff, cfrac)
    ...
    output_file_path=get_validation_dict_name(target_beff, cfrac, nJets, train_config.model_name)
