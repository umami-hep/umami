    ...
    if config.Cuts is not None:
        for elem in config.Cuts:
            ...
            if config.Cuts[elem]["NaNcheck"] is True:
                indices_i_to_remove = np.where(np.logical_not(op_func(
                    jets[elem], config.Cuts[elem]['condition'])) & (
                        jets[elem] == jets[elem]))[0]
            else:
                indices_i_to_remove = np.where(np.logical_not(op_func(
                    jets[elem], config.Cuts[elem]['condition'])))[0]
        ...
        if config.bhad_pTcut is not None:
            indices_to_remove_bjets = np.where(
                (jets["HadronConeExclTruthLabelID"] == 5) & (
                    jets["GhostBHadronsFinalPt"] > config.bhad_pTcut))[0]
