...
from umami.tools import yaml_loader, replaceLineInFile
...
def getConfiguration():
    ...
    for key in ['data_url', 'test_preprocessing']:
        if key not in conf_setup.keys(): raise yaml.YAMLError(f"Missing key in yaml file ({path_configuration}): {key}")
    return conf_setup
...
def runPreprocessing(config, var_dict, scale_dict, output):
    ...
    run_undersampling = run(['preprocessing.py', '-c', f'{config}', '--var_dict', var_dict, '--undersampling'])
    try:
    ...
    run_scaling = run(['preprocessing.py', '-c', f'{config}', '--var_dict', var_dict, '--scaling'])
    try:
    ...
    run_apply_scales = run(['preprocessing.py', '-c', f'{config}', '--var_dict', var_dict, '--apply_scales'])
    try:
    ...
    logging.info("Test: shuffling the samples and writing the samples to disk...")
    run_write = run(['preprocessing.py', '-c', f'{config}', '--var_dict', var_dict, '--write'])
    try:
...
class TestPreprocessing(unittest.TestCase):
    def setUp(self):
        ...
        if test_dir.startswith('/tmp'): run(['rm', '-rf', test_dir])
        run(['mkdir', '-p', test_dir])

        # inputs for test will be located in test_dir
        config_source = os.path.join(os.getcwd(), self.data['test_preprocessing']['config'])
        var_dict_dl1r_source = os.path.join(os.getcwd(), self.data['test_preprocessing']['var_dict_dl1r'])
        var_dict_dips_source = os.path.join(os.getcwd(), self.data['test_preprocessing']['var_dict_dips'])
        var_dict_umami_source = os.path.join(os.getcwd(), self.data['test_preprocessing']['var_dict_umami'])
        self.config = os.path.join(test_dir, os.path.basename(config_source))
        self.var_dict_dl1r = os.path.join(test_dir, os.path.basename(var_dict_dl1r_source))
        self.var_dict_dips = os.path.join(test_dir, os.path.basename(var_dict_dips_source))
        self.var_dict_umami = os.path.join(test_dir, os.path.basename(var_dict_umami_source))
        self.scale_dict = os.path.join(test_dir, 'PFlow-scale_dict.json')
        self.output = os.path.join(test_dir, 'PFlow-hybrid_70-test.h5')

        logging.info(f'Preparing config file based on {config_source} in {self.config}...')
        ...
        replaceLineInFile(self.config, 'file_path:', f'  file_path: &file_path {test_dir}')
        replaceLineInFile(self.config, 'outfile_name:', f'outfile_name: {self.output}')
        replaceLineInFile(self.config, 'dict_file:', f'dict_file: {self.scale_dict}')
        ...
        for file in self.data['test_preprocessing']['files']:
            path = os.path.join(self.data['data_url'], self.data['test_preprocessing']['data_subfolder'], file)
    ...
    def test_preprocessing_dl1r(self):
        ...
        self.assertTrue(runPreprocessing(self.config, self.var_dict_dl1r, self.scale_dict, self.output))


    def test_preprocessing_dips(self):
        """Integration test of preprocessing.py script using DIPS variables."""
        self.assertTrue(runPreprocessing(self.config, self.var_dict_dips, self.scale_dict, self.output))


    def test_preprocessing_umami(self):
        """Integration test of preprocessing.py script using Umami variables."""
        self.assertTrue(runPreprocessing(self.config, self.var_dict_umami, self.scale_dict, self.output))
