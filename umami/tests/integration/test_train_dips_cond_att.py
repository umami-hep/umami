...
def runTrainingDips(config):
    ...
    if isSuccess is True:
        run_train_Dips_cond_att

    logger.info(
        "Test: running plotting_epoch_performance.py for DIPS Conditional Attention..."
    )
    ...
    try:
        ...
        logger.info(
            "Test failed: plotting_epoch_performance.py for DIPS Conditional Attention."
        )
        isSuccess = False
...
class TestDipsTraining(unittest.TestCase):
    def setUp(self):
        ...
        config_source = os.path.join(
            os.getcwd(), self.data["test_dips_cond_att"]["config"]
        )
        ...
        for file in self.data["test_dips_cond_att"]["files"]:
            path = os.path.join(
                self.data["data_url"],
                self.data["test_dips_cond_att"]["data_subfolder"],
                file,
            )
