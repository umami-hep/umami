...
def getArgumentParser():
    ...
    parser.add_argument('-f', '--fractions', default=[0., 0.018, 0.982])
...
def main():
    ...
    for jet_class, fraction in zip(class_labels, args.fractions):
        if abs(fraction) > 0.: frac_dict[jet_class] = fraction
    ...
    variables += [
        ...
        global_config.pTvariable, global_config.etavariable
    ]
    ...
    for eff_wp in args.working_points:
        d_b_cutvalue = np.quantile(d_b, 1. - float(eff_wp))
